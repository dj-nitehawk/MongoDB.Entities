...
public partial class DBContext
{
    ...
    public Task<DeleteResult> DeleteAsync<T>(object? ID, CancellationToken cancellation = default, bool ignoreGlobalFilters = false) where T : IEntity
    {
        return DB.DeleteAsync(
            Logic.MergeWithGlobalFilter(ignoreGlobalFilters, globalFilters, Builders<T>.Filter.Eq(Cache<T>.Get(typeof(T)).IdPropName, ID)),
            Session,
            cancellation);
    }
    ...
    {
        return DB.DeleteAsync(
            Logic.MergeWithGlobalFilter(ignoreGlobalFilters, globalFilters, Builders<T>.Filter.In(Cache<T>.Get(typeof(T)).IdPropName, IDs)),
            Session,
            cancellation);
    }
