...
public class TransactionsEntity
{
    ...
    public async Task not_commiting_and_aborting_update_transaction_doesnt_modify_docs()
    {
        ...
        var res = await DBInstance.Instance().Find<AuthorEntity>().OneAsync(author1.ID);
    ...
    public async Task delete_in_transaction_works()
    {
        ...
        Assert.AreEqual(null, await DBInstance.Instance().Find<BookEntity>().OneAsync(book1.ID));
    }

    [TestMethod]
    public async Task full_text_search_transaction_returns_correct_results()
    {
        await DBInstance.Instance().Index<AuthorEntity>()
    ...
    public async Task bulk_save_entities_transaction_returns_correct_results()
    {
        ...
        var res = await DBInstance.Instance().Find<BookEntity>().ManyAsync(b => b.Title.Contains(guid));
        ...
        res = await DBInstance.Instance().Find<BookEntity>().ManyAsync(b => b.Title.Contains(guid));
