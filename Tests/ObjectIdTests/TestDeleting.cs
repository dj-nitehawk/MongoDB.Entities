...
public class DeletingObjectId
{
    ...
    public async Task delete_by_expression_deletes_all_matchesAsync()
    {
        ...
        await DBInstance.Instance().DeleteAsync<AuthorObjectId>(x => x.Name == "xxx");

        var count = await DBInstance.Instance().Queryable<AuthorObjectId>()
    ...
    public async Task delete_by_ids_with_global_filter()
    {
        ...
        var notDeletedIDs = await DBInstance.Instance().Find<AuthorObjectId, ObjectId?>()
