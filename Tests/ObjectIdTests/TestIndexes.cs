...
public class IndexesObjectId
{
    ...
    {
        await DBInstance.Instance().DropCollectionAsync<AuthorObjectId>();

        await DBInstance.Instance().Index<AuthorObjectId>()
        ...
        var res = DBInstance.Instance().FluentTextSearch<AuthorObjectId>(Search.Full, author1.Surname).ToList();
        Assert.AreEqual(author1.Surname, res[0].Surname);

        var res2 = await DBInstance.Instance().Find<AuthorObjectId>()
    ...
    public async Task full_text_search_with_wilcard_text_index_works()
    {
        ...
        var res = await DBInstance.Instance().FluentTextSearch<AuthorObjectId>(Search.Full, author1.Surname).ToListAsync();
    ...
    public async Task fuzzy_text_search_with_text_index_works()
    {
        ...
        await dbInstance.Index<BookObjectId>()
          .Option(o => o.Background = false)
        ...
        var res = await dbInstance.Find<BookObjectId>()
        ...
        await dbInstance.DeleteAsync<BookObjectId>(new object[] { b1.ID, b2.ID, b3.ID, b4.ID, b5.ID, b6.ID });
    ...
    {
        await DBInstance.Instance().DropCollectionAsync<GenreObjectId>();

        await DBInstance.Instance().Index<GenreObjectId>()
        ...
        var res = await DBInstance.Instance().Find<GenreObjectId>()
    ...
    {
        await DBInstance.Instance().Index<BookObjectId>()
