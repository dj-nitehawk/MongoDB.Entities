...
public class TransactionsUuid
{
    ...
    public async Task not_commiting_and_aborting_update_transaction_doesnt_modify_docs()
    {
        ...
        var author1 = new AuthorUuid { Name = "uwtrcd1", Surname = guid };
        await author1.SaveAsync();
        var author2 = new AuthorUuid { Name = "uwtrcd2", Surname = guid };
        await author2.SaveAsync();
        var author3 = new AuthorUuid { Name = "uwtrcd3", Surname = guid };
        ...
        var res = await DBInstance.Instance().Find<AuthorUuid>().OneAsync(author1.ID);
    ...
    public async Task delete_in_transaction_works()
    {
        ...
        Assert.AreEqual(null, await DBInstance.Instance().Find<BookUuid>().OneAsync(book1.ID));
    }

    [TestMethod]
    public async Task full_text_search_transaction_returns_correct_results()
    {
        await DBInstance.Instance().Index<AuthorUuid>()
        ...
        await DBInstance.Instance().SaveAsync(author1);
        await DBInstance.Instance().SaveAsync(author2);
    ...
    public async Task bulk_save_entities_transaction_returns_correct_results()
    {
        ...
        var res = await DBInstance.Instance().Find<BookUuid>().ManyAsync(b => b.Title.Contains(guid));
        ...
        res = await DBInstance.Instance().Find<BookUuid>().ManyAsync(b => b.Title.Contains(guid));
