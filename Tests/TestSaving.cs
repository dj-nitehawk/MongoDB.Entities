...
public class Saving
{
    ...
    public async Task save_preserving_inverse_attribute()
    {
        var book = new Book
        {
            ...
            PriceDbl = 666,
            MainAuthor = new(new Author{ ID = ObjectId.GenerateNewId().ToString() })
        };
        await book.SaveAsync();
    ...
    public async Task save_preserving_attribute()
    {
        ...
        author.BestSeller = new(new Book{ ID = ObjectId.GenerateNewId().ToString()});
    ...
    public async Task embedding_non_entity_returns_correct_document()
    {
        ...
        {
            Title = "Test",
            Review = new Review { Stars = 5, Reviewer = "enercd" }
        };
        await book.SaveAsync();
