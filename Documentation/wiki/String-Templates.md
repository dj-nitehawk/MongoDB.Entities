...
The mongodb driver has its limits and sometimes you need to compose queries either by hand or using a query editor/composer. To be able to run those queries and inject values from your C# code, you're required to compose `BsonDocuments` or do string concatenation which leads to an ugly, unreadable, magic string infested mess.
...
The `.Tag()` method simply replaces matching tags on the text query with the supplied value. You don't have to use the `<` and `>` characters while using the `.Tag()` method. In fact, avoid it as the tags won't match if you use them.
...
| Expression                         | Resulting path        |
|------------------------------------|-----------------------|
| `x => x.Authors[0].Books[0].Title` | `Authors.Books.Title` |
...
These kinds of runtime errors are preferable than your code failing silently because the queries didn't produce any results or produced the wrong results.
