...
IDs for new entities are generated by the mongodb driver's `IIdGenerator` resolved for the entity's ID property. Out of the box, `string` IDs get ObjectId-formatted strings, and `ObjectId` and `Guid` IDs get new ObjectIds/Guids. If you'd like to change the type of the ID, simply declare the ID property with the type you want (decorated with `[BsonId]` when implementing `IEntity`):
...
To customize how ID values are generated, register an `IIdGenerator` either for a single entity type (works for any entity, including ones inheriting their ID property from the `Entity` base class):

```csharp
DB.RegisterIdGenerator<Book>(new MyCustomIdGenerator());
```

or for the CLR type of the ID (affects all entities with that ID type):
