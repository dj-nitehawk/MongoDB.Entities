...
- **Cannot decorate inherited `Entity.ID`:** to keep BSON ObjectId `_id`s, register a class map with `StringSerializer(BsonType.ObjectId)` or implement `IEntity` with `[AsObjectId]`/`[ObjectId]` on your own ID property (see changelog migration notes).
